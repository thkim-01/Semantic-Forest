...
class SemanticDecisionTree:
    ...
    def get_feature_importance(self) -> dict:
        """Feature importance 계산 (refinement별 정보 이득 기여도)"""
        importance = {}
        ...
        for node in self.nodes:
            if not node.is_leaf and node.refinement:
                ...
                if node.left_child and node.right_child:
                    ...
                    info_gain -= (n_left / n) * node.left_child.entropy
                    info_gain -= (n_right / n) * node.right_child.entropy
                    
                    if prop not in importance:
                        importance[prop] = 0.0
                    importance[prop] += info_gain
