...
class SDTLearner:
    ...
    def _calculate_information_gain(self, node: TreeNode, refinement: Refinement) -> float:
        ...
        # 현재 노드 엔트로피 (Weighted if set)
        node_entropy = self._calculate_entropy(node.instances)
        # Note: We re-calculate node_entropy here because node.entropy stored in TreeNode might be unweighted
        ...
        information_gain = node_entropy - weighted_child_entropy
        
        return information_gain
    
    def _calculate_entropy(self, instances: List) -> float:
        """인스턴스 집합의 엔트로피 계산 (Weighted support)"""
        if len(instances) == 0:
            return 0.0
